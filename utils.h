...
public:
    /**
     * Lets you iterate over a python list.
     */
    static void iterateList(PyObject* list, std::function<void(int, PyObject*)> callback)
    {
        ...
        {
            callback(i, PyList_GET_ITEM(list, i));
        }
    }

    /**
     * Returns the i-th entry of a tuple.
     */
    static PyObject* getTupleEntry(int i, PyObject* tuple)
    {
        ...
        if (PyTuple_GET_SIZE(tuple) <= i)
        {
            throw ConversionException("Index out of bounds.");
        }
